...
$.fn.accordionShow = function() {
  const block = this;
  const content = block.find('.js-accordion-content');
  const isMultiple = block.data('accordion-single');
  ...
  if (isMultiple) {
    const siblings = block.siblings('.js-accordion.is-active');
...
export default function accordion() {
  $(document).on('click', '.js-accordion-button', function(e) {
    ...
    const block = $(this).parents('.js-accordion');
