function putBlockIntoSlot() {
  $('[data-target-slot]').each(function() {
    ...
    const slot = slots.filter(function() {
      ...
      if (
        vw < window.globalOptions.sizes.xs &&
        slots.filter('[data-slot-res="xs"]').length
      ) {
        return res === 'xs';
      }
      if (
        vw < window.globalOptions.sizes.sm &&
        slots.filter('[data-slot-res="sm"]').length
      ) {
        return res === 'sm';
      }
      if (
        vw < window.globalOptions.sizes.md &&
        slots.filter('[data-slot-res="md"]').length
      ) {
        return res === 'md';
      }
      if (
        vw < window.globalOptions.sizes.lg &&
        slots.filter('[data-slot-res="lg"]').length
      ) {
        return res === 'lg';
      }
